---
title: "Wales Early Stage Diagnosis Cleaning Script"
author: "Matt Grant"
date: "2023-06-27"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
library(tidyverse)
library(readxl)

```

# Load in data

```{r}
walesED <- read_excel("CancerIncidenceInWales_DataTables, 2002-2019_v2 (1).xlsx", 
    sheet = "Table 3 - Staging")

```

```{r}
wales_17_19 <- walesED %>%
  filter(Year == "2017-2019")

wales_17_19 <- wales_17_19 %>%
  #filter(AGE > 34) %>%
  mutate(ICD10 = case_when(`Cancer type` == "Lip, oral cavity and pharynx" 
                                              ~ "C00-C14",
                           `Cancer type` == "Head & neck" ~ "C00-C14, C30-C32",
                           `Cancer type` == "Oesophagus" ~ "C15",
                           `Cancer type` == "Stomach" ~ "C16",
                           `Cancer type` == "Colon" ~ "C18",
                           `Cancer type` == "Colorectal" ~ "C18-C20",
                           `Cancer type` == "Rectum" ~ "C19-C20",
                           `Cancer type` == "Anus" ~ "C21",
                           `Cancer type` == "Liver" ~ "C22",
                           `Cancer type` == "Hepatocellular carcinoma" ~ "C220",
                           `Cancer type` == "Cholangiocarcinoma" ~ "C221",
                           `Cancer type` == "Pancreas" ~ "C25",
                           `Cancer type` == "Larynx" ~ "C32",
                           `Cancer type` == "Lung" ~ "C33-C34",
                           `Cancer type` == "Bone" ~ "C40-C41",
                           `Cancer type` == "Melanoma" ~ "C43",
                           `Cancer type` == "Mesothelioma" ~ "C45",
                           `Cancer type` == "Breast" ~ "C50",
                           `Cancer type` == "Female breast" ~ "C50",
                           `Cancer type` == "Cervix" ~ "C53",
                           `Cancer type` == "Uterus" ~ "C54",
                           `Cancer type` == "Ovary" ~ "C56",
                           `Cancer type` == "Prostate" ~ "C61",
                           `Cancer type` == "Testis" ~ "C62",
                           `Cancer type` == "Kidney" ~ "C64",
                           `Cancer type` == "Urinary tract excluding bladder" 
                                              ~ "C64-C66,C68",
                           `Cancer type` == "Bladder" ~ "C67",
                           `Cancer type` == "Thyroid" ~ "C73",
                           `Cancer type` == "Hodgkin lymphoma" ~ "C81",
                           `Cancer type` == "Non-Hodgkin lymphoma" ~ "C82-C86",
                           `Cancer type` == "Myeloma" ~ "C90")) %>%
  filter(Sex == "Persons")

sum(is.na(wales_17_19$ICD10))
```

Several cancer types are covered by groupings in the data and so will need to be removed:

| Cancer type                  | Code    | Covered by                      | Code             |
|------------------------------|---------|---------------------------------|------------------|
| Lip, oral cavity and pharynx | C00-C14 | Head & neck                     | C00-C14, C30-C32 |
| Colon                        | C18     | Colorectal                      | C18-C20          |
| Rectum                       | C19-C20 | Colorectal                      | C18-C20          |
| Hepatocellular carcinoma     | C220    | Liver                           | C20              |
| Cholangiocarcinoma           | C221    | Liver                           | C20              |
| Larynx                       | C32     | Head & neck                     | C00-C14, C30-C32 |
| Female breast                | C50     | Breast                          | C50              |
| Kidney                       | C64     | Urinary tract excluding bladder | C64-C66, C68     |

Note: Breast C50 is only provided for Wales as a whole, for health boards we need to use Female breast C50 instead.

# Calculating Early Stage proportions

## Wales

```{r}
#Filter to wales only
wales_only_17_19 <- wales_17_19 %>%
  filter(Geography == "Wales")

#Filter out duplicate cancer types
wales_only_17_19 <- wales_only_17_19 %>%
  filter(`Cancer type` != "Lip, oral cavity and pharynx",
         `Cancer type` != "Colon",
         `Cancer type` != "Rectum",
         `Cancer type` != "Hepatocellular carcinoma",
         `Cancer type` != "Cholangiocarcinoma",
         `Cancer type` != "Larynx",
         `Cancer type` != "Female breast",
         `Cancer type` != "Kidney") 
         
           

#Calculate the total number of diagnoses with KNOWN stage for each cancer type
total_cancer_cases <- wales_only_17_19 %>%
  filter(Stage == "Unknown") %>%
  mutate(`Total count` = `Total count` - Count) %>%
  select(`Cancer type`, `Total count`)

#Calculate the total num
stage_1_2_count <- wales_only_17_19 %>%
  filter(Stage == "1" | Stage == "2") %>%
  group_by(`Cancer type`) %>%
  summarise(stage_1_2 = sum(Count))

early_stage_diag <- merge(stage_1_2_count, 
                          total_cancer_cases, 
                          by = "Cancer type") %>%
  mutate(proportion = stage_1_2 / `Total count`)

#Calculate the total number of cases diagnosed with KNOWN stage
diag_total = sum(early_stage_diag$`Total count`)
#Calculate the total number of stage 1 and 2 diagnoses  
early_stage_diag_total = sum(early_stage_diag$stage_1_2)

#Calculate final proportion
proportion_early_diag = early_stage_diag_total / diag_total
proportion_early_diag
```

## South and Mid Wales

```{r}
s_wales_17_19 <- wales_17_19 %>%
  filter(`Geography level` == "Health board") %>%
  filter(Geography == "Aneurin Bevan UHB" | 
            Geography == "Cardiff and Vale UHB" |
            Geography == "Cwm Taf Morgannwg UHB" |
            Geography == "Hywel Dda UHB" |
            Geography == "Powys THB" |
            Geography == "Swansea Bay UHB")

s_wales_17_19 <- s_wales_17_19 %>%
  filter(`Cancer type` != "Lip, oral cavity and pharynx",
         `Cancer type` != "Colon",
         `Cancer type` != "Rectum",
         `Cancer type` != "Hepatocellular carcinoma",
         `Cancer type` != "Cholangiocarcinoma",
         `Cancer type` != "Larynx",
         `Cancer type` != "Kidney") 

#Calculate the total number of diagnoses with KNOWN stage for each cancer type
sw_total_cancer_cases <- s_wales_17_19 %>%
  filter(Stage == "Unknown") %>%
  mutate(`Total count` = `Total count` - Count) %>%
  select(`Cancer type`, `Total count`)

#Calculate the total num
sw_stage_1_2_count <- s_wales_17_19 %>%
  filter(Stage == "1" | Stage == "2") %>%
  group_by(`Cancer type`) %>%
  summarise(stage_1_2 = sum(Count))

sw_early_stage_diag <- merge(sw_stage_1_2_count, 
                          sw_total_cancer_cases, 
                          by = "Cancer type") %>%
  mutate(proportion = stage_1_2 / `Total count`)

#Calculate the total number of cases diagnosed with KNOWN stage
sw_diag_total = sum(sw_total_cancer_cases$`Total count`)
#Calculate the total number of stage 1 and 2 diagnoses  
sw_early_stage_diag_total = sum(sw_stage_1_2_count$stage_1_2)

#Calculate final proportion
sw_proportion_early_diag = sw_early_stage_diag_total / sw_diag_total

sw_proportion_early_diag
```

## North Wales

```{r}
n_wales_17_19 <- wales_17_19 %>%
  filter(`Geography level` == "Health board") %>%
  filter(Geography == "Betsi Cadwaladr UHB")

n_wales_17_19 <- n_wales_17_19 %>%
  filter(`Cancer type` != "Lip, oral cavity and pharynx",
         `Cancer type` != "Colon",
         `Cancer type` != "Rectum",
         `Cancer type` != "Hepatocellular carcinoma",
         `Cancer type` != "Cholangiocarcinoma",
         `Cancer type` != "Larynx",
         `Cancer type` != "Kidney") 

#Calculate the total number of diagnoses with KNOWN stage for each cancer type
total_cancer_cases_nw <- n_wales_17_19 %>%
  filter(Stage == "Unknown") %>%
  mutate(`Total count` = `Total count` - Count) %>%
  select(`Cancer type`, `Total count`)

#Calculate the total num
nw_stage_1_2_count <- n_wales_17_19 %>%
  filter(Stage == "1" | Stage == "2") %>%
  group_by(`Cancer type`) %>%
  summarise(stage_1_2 = sum(Count))

nw_early_stage_diag <- merge(nw_stage_1_2_count, 
                          total_cancer_cases_nw, 
                          by = "Cancer type") %>%
  mutate(proportion = stage_1_2 / `Total count`)

#Calculate the total number of cases diagnosed with KNOWN stage
nw_diag_total = sum(total_cancer_cases_nw$`Total count`)
#Calculate the total number of stage 1 and 2 diagnoses  
nw_early_stage_diag_total = sum(nw_stage_1_2_count$stage_1_2)

#Calculate final proportion
nw_proportion_early_diag = nw_early_stage_diag_total / nw_diag_total
nw_proportion_early_diag
```
